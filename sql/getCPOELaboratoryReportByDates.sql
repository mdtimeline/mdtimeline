DROP PROCEDURE IF EXISTS `getCPOELaboratoryReportByDates`;


DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `getCPOELaboratoryReportByDates`(IN provider_id INT, IN start_date DATE, IN end_date DATE, IN stages VARCHAR(40))
BEGIN
    ...
    FROM
        (SELECT ord.id, ord.pid, ord.priority FROM patient_orders as ord
         ...
         WHERE e.provider_uid = provider_id AND ord.order_type = 'lab' AND ord.date_ordered BETWEEN start_date AND end_date
         GROUP BY ord.id) o;
