...
//		$types['TQ'][1] = $types['CQ'];      // Quantity (CQ)
		...
		$types['TXA'][0] = '';               // (TXA)
		$types['TXA'][1] = $types['ST'];     // ID Number (ST)
		$types['TXA'][2] = $types['FN'];     // Family Name (FN)
		$types['TXA'][3] = $types['ST'];     // Given Name (ST)
		$types['TXA'][4] = $types['ST'];     // Second and Further Given Names or Initials Thereof (ST)
		$types['TXA'][5] = $types['ST'];     // Suffix (e.g., JR or III) (ST)
		$types['TXA'][6] = $types['ST'];     // Prefix (e.g., DR) (ST)
		$types['TXA'][7] = $types['IS'];     // DEPRECATED-Degree (e.g., MD) (IS)
		$types['TXA'][8] = $types['IS'];     // Source Table (IS)
		$types['TXA'][9] = $types['HD'];     // Assigning Authority (HD)
		$types['TXA'][10] = $types['ID'];    // Name Type Code (ID)
		$types['TXA'][11] = $types['ST'];    // Identifier Check Digit (ST)
		$types['TXA'][12] = $types['ID'];    // Check Digit Scheme (ID)
		$types['TXA'][13] = $types['ID'];    // Identifier Type Code (ID)
		$types['TXA'][14] = $types['HD'];    // Assigning Facility (HD)
		$types['TXA'][15] = $types['TS'];    // Date/Time Action Performed (TS)
		$types['TXA'][16] = $types['ID'];    // Name Representation Code (ID)
		$types['TXA'][17] = $types['CE'];    // Name Context (CE)
		$types['TXA'][18] = $types['DR'];    // DEPRECATED-Name Validity Range (DR)
		$types['TXA'][19] = $types['ID'];    // Name Assembly Order (ID)
		$types['TXA'][20] = $types['TS'];    // Effective Date (TS)
		$types['TXA'][21] = $types['TS'];    // Expiration Date (TS)
		$types['TXA'][22] = $types['ST'];    // Professional Suffix (ST)
		$types['TXA'][23] = $types['CWE'];   // Assigning Jurisdiction (CWE)
		$types['TXA'][24] = $types['CWE'];   // Assigning Agency or Department (CWE)
		...
		$types['SN'][3] = $types['ST'];     // (ST)
		$types['SN'][4] = $types['NM'];     // (NM)
