...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `getAdvanceCarePlanReportByDates`(IN provider_id INT, IN insurance_id INT, IN start_date DATE, IN end_date DATE, IN sex CHAR, IN ethnicity VARCHAR(40), IN race VARCHAR(40))
BEGIN
...
#     SET @end_date = '2024-01-01';
    ...
    END IF;
        ...
        SELECT 1 as `value`, pid FROM report_ds GROUP BY eid;

        CREATE TEMPORARY TABLE report_numerator_ds
        SELECT 1 as `value`, pid FROM report_ds WHERE has_advance_directive = '1' GROUP BY eid;
