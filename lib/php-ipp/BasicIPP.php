...
class BasicIPP
{
	...
	protected function _parseServerOutput()
	{
		...
		$status = isset($this->serveroutput->status) ? $this->serveroutput->status : 'UNKNOW';

		self::_putDebug(
			sprintf("***** IPP STATUS: %s ******", $status), 4);
	...
	protected function _parseIppVersion()
	{
		...
		if(
			$this->serveroutput->body != '' &&
			isset($this->serveroutput->body[$this->_parsing->offset]) &&
			isset($this->serveroutput->body[$this->_parsing->offset + 1])
		){
			$major = ord($this->serveroutput->body[$this->_parsing->offset]) * 256;
			$minor = ord($this->serveroutput->body[$this->_parsing->offset + 1]);
		}else{
			$major = 0;
			$minor = 0;
		}
		...
		switch ($ippversion)
		{
			...
			default:
				$this->serveroutput->ipp_version =
					sprintf("%u.%u (Unknown)",$major,$minor);
				break;
		}
		self::_putDebug("I P P    R E S P O N S E :\n\n");
		self::_putDebug(
			sprintf("IPP version %s%s: %s", $major, $minor, $this->serveroutput->ipp_version));
