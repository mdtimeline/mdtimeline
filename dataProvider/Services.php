...
class Services {
	...
	public function getServicesQuickPickFieldsBySpecialty($specialty_id){
...
TXT;
		...
		foreach($services as $service_group => $services_fields){

			$field = [
				'xtype' => 'fieldset',
				'title' => (string) $service_group,
				'items' => [
					[
						'xtype' => 'checkboxgroup',
						'columns' => 2,
			...
			foreach($services_fields as $cpt_code => $cpt_description){

				$cpt_description = (string) $cpt_description;
				$cpt_code = (string) $cpt_code;

				$field['items'][0]['items'][] = [
					'xtype' => 'checkbox',
					'boxLabel' => "[$cpt_code] $cpt_description",
					'inputValue' => "CPT:$cpt_code:$cpt_description",
					'name' => 'services',
					'value' => $cpt_code
				];
			}
