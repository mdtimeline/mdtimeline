				...
				if (is_file($file)) {
					if ($this->codeType == 'RXNORM') {
						...
						if (preg_match("/SnomedCT_RF1Release_INT_([0-9]{8}).zip/", $file, $matches)) {
							...
							array_push($revisions, $temp_date);
						} elseif (preg_match("/SnomedCT_USEditionRF2_PRODUCTION_([0-9]{8})T120000Z.zip/", $file, $matches)) {
				...
				if (is_dir($idc10Dir)) {
					if ($this->file->setWorkingDir()) {
						...
						foreach ($files as $file) {
							if ($file == $params->version . '_PCS_long_and_abbreviated_titles.zip' ||
	...
	private function snomed_import($dir)
	{
		...
		$table_array_for_snomed = [
			...
			'sct_descriptions_structure' => 'CREATE TABLE IF NOT EXISTS sct_descriptions (
	            ...
	            CaseSignificanceId INT(20) NOT NULL,
			...
			'sct_relationships_structure' => 'CREATE TABLE IF NOT EXISTS sct_relationships (
	            ...
	            INDEX X_Active(Active),
	            INDEX X_ModuleId(ModuleId),
	            INDEX X_SourceId(SourceId),
	            INDEX X_DestinationId(DestinationId),
	            INDEX X_RelationshipGroup(RelationshipGroup),
	            INDEX X_TypeId(TypeId)
	            )',
		...
		if (is_dir($dir . $sub_path) && $handle = opendir($dir . $sub_path)) {
			...
			while (false !== ($filename = readdir($handle))) {
				if ($filename != '.' && $filename != '..' && !strpos($filename, 'zip')) {
					...
					if (strpos($filename, 'Concept') !== false) {
						$new_str = str_replace($array_replace, [
							$path,
							'sct_concepts'
						], $load_script);
					} elseif (strpos($filename, 'Description') !== false) {
						$new_str = str_replace($array_replace, [
							$path,
							'sct_descriptions'
						], $load_script);
					} elseif (strpos($filename, 'Relationship') !== false) {
						$new_str = str_replace($array_replace, [
							$path,
							'sct_relationships'
						], $load_script);
					...
					if (isset($new_str)) {
						$success = $this->db->conn()->exec($new_str);
					}
				}
			}
			closedir($handle);
